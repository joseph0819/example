...
const Advisor = () => {
  return (
    ...
    <div className='container px-4 md:px-6 xl:px-0 xl:max-w-[1140px] 3xl:max-w-[1580px] mx-auto'>
        <h3 className='text-blue text-center font-bold text-[30px] sm:text-4xl lg:text-5xl  3xl:text-6xl'>Advisory</h3>
        <p className='text-light_gray text-center text-[15px] max-w-[625px] mx-auto'>Our success stems from the leadership, professionalism and vision of our management and staff.</p>
        <div className='flex justify-center items-center gap-6 rounded-lg mt-8 flex-wrap'>
          ...
          {AdvisorData.map((obj, index) => (
            <div className='w-[90%] md:w-[48%] xl:w-[23%] group'>
                <div className='overflow-hidden'><img className='w-full group-hover:scale-125 duration-300' src={obj.img} alt="img" /></div>
                <div className='flex items-center justify-between mt-4'>
                    <p className='ff_jost font-bold 2xl:text-xl'>{obj.heading}</p>
                   <a href={obj.link}><Linkdin/></a>
                </div>
            </div>
             ))}
          </div>
